...
"DISKSPACE",	"GETENV",	"GETLOGIN",	"HBCURL",	"MEMORY",	
"REGEX",	"SYSTEM",	"URL2DOMAIN",	"URLDECODE",	"URLENCODE",	"VERSION",	"VRAND"
};

static int fcn_bin_srch(char *fcn, int start, int end)
{
...
static int diskspace(HBCONN conn, int parm_cnt)
{
	int dr = 0;
	long ds;
#ifdef PARMCHK
...
char *url2dn(char *url)
{
	...
	if (tp2 = strchr(tp, '/')) sprintf(url2, "%.*s", tp2 - tp, tp);
	else if (tp2 = strchr(tp, '?')) sprintf(url2, "%.*s", tp2 - tp, tp);
	else {
	...
	sprintf(dn, "%.*s", tp2 - tp, tp);
	if (!istopdomain(dn)) return NULL;
	tt = tp - 1;
	while (tt >= url2 && *tt != '.') --tt;
	if (tt <= url2) return NULL;
	if (tp2-tt >= MAXDNLEN) return NULL;
	sprintf(dn, "%.*s", tp - tt, tt);
	if (!istopdomain(dn)) {
		sprintf(dn, "%.*s", tp2 - tt - 1, tt + 1);
		return dn;
	}
	...
	if (tp2 - tt2 >= MAXDNLEN) return NULL;
	sprintf(dn, "%.*s", tp2 - tt2 - 1, tt2 + 1);
	return dn;
}
...
static int urldecode(HBCONN conn, int parm_cnt)
{
	...
	*tp2 = '\0';
	return(dbe_push_string(conn, url, tp2 - url));
}

#define MAXNUMLEN	20

static int vrand(HBCONN conn, int parm_cnt)
{
	char vbuf[STRBUFSZ];
	HBVAL *v = (HBVAL *)vbuf;
	int len, i;
...
#endif
	if (!dbe_peek_stack(conn, parm_cnt - 1, CHARCTR, vbuf, STRBUFSZ)) return(-1);
	switch (v->buf[0]) {
	case 'c':
	case 'C':
		len = dbe_pop_int(conn, 1, MAXSTRLEN, NULL);
		dbe_popstack(conn, vbuf, STRBUFSZ);	/** pop off the 'c' or 'C' **/
		for (i=0; i<len; ++i) v->buf[i] = (int)(84.0 * rand() / (RAND_MAX + 1.0)) + 32;
		dbe_push_string(conn, v->buf, len);
		break;
	case 'n':
	case 'N': {
		int deci = 0;
		double rn;
		if (parm_cnt == 3) deci = dbe_pop_int(conn, 0, MAXNUMLEN - 1, NULL);
		len = dbe_pop_int(conn, 1, MAXNUMLEN, NULL);;
		if (deci > len - 1) {
			hb_error(conn, INVALID_VALUE, NULL, 0, 0);
			return(-1);
		}
		dbe_popstack(conn, vbuf, STRBUFSZ);	/** pop off the 'n' or 'N' **/
		if (deci) --len;
		rn = (pow(10.0,(double)len) - 1.0) * rand() / (RAND_MAX + 1.0);
		if (deci) rn /= pow(10.0, (double)deci);
		dbe_push_real(conn, rn);
		break; }
	case 'd':
	case 'D': {
		date_t d1, d2, elapsed;
		dbe_popstack(conn, vbuf, STRBUFSZ);
		if (v->valspec.typ != DATE) {
			hb_error(conn, INVALID_VALUE, NULL, 0, 0);
			return(-1);
		}
		d2 = get_date(v->buf);
		...
		if (d1 > d2) {
			date_t tmp = d1; d1 = d2; d2 = tmp;
		}
		elapsed = d2 - d1;
		dbe_popstack(conn, vbuf, STRBUFSZ);	/** pop off the 'd' or 'D' **/
		dbe_push_date(conn,(date_t)((double)elapsed * rand() / (RAND_MAX + 1.0)) + d1);
		break; }
	default:
		hb_error(conn, INVALID_VALUE, NULL, 0, 0);
		return(-1);
...
{
	FILE *fpcurl = NULL;
	char url[STRBUFSZ], data[STRBUFSZ], cmd[1024], *tp;
#ifdef PARMCHK
	if (parm_cnt != 2)  {
		hb_error(conn, INVALID_PARMCNT, NULL, 0, 0);
		goto out;
...
static int system_(HBCONN conn, int parm_cnt) //only called when SHELL_SERVER is not set on server side, see fcn.c
{
	FILE *pcmd;
	...
	while (fgets(line, MAXLINELEN, pcmd)) hbscrn_puts(line);
	status = pclose(pcmd);
...
int (*hb_usrfcn[NFCNS])() = {
diskspace,	getenv_,	getlogin_,	hbcurl,	
regex_match,	system_,	url2domain,	urldecode,	urlencode,	version,	vrand,
};
